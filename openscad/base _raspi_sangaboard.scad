...
module base_projection(){
    projection(cut=true){
        ...
        union(){    // casing around the leg&lever
            ...
            translate([-(casing_radius+1),0,0])cube([(casing_radius+1),casing_apothem,10]);
            
        }
        translate([-(casing_radius+1),2.5,0])cube([(casing_radius+1),casing_apothem,10]);
        translate(raspi_center+[0,-(raspi_board[1]+2),-raspi_standoff])cube(raspi_board+[10,0,0],center=true);
    }
...
module base_windowed() {
    ...
    difference() {
        base();
        window_cubes();
        #translate([-(casing_radius+2),0,base_height-3])cube([casing_radius+2,sangaboard_center[1]+sangaboard[1]/2-sangaboard_window[1]/2,10]);
        translate(raspi_center+[0,-(raspi_board[1]+2),base_height-3-raspi_standoff])cube(raspi_board+[10,0,0],center=true);
    }
